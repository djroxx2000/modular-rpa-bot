		...
		playback: (function () {
			...
			*/
			var constructor = function (testRunURL) {
				var self = this;
				console.log(JSON.parse(testRunURL));
				let data = JSON.parse(testRunURL);
				this.window = data.window;
				this.userEventLog = data.event_log;
			...
			constructor.prototype = {
				...
				*/
				start: function () {
					var self = this;
					console.log('start test: ', this.window);
					...
					var runSimulator = setInterval(function () {
						...
						if (currentTime > self.userEventLog[0].timeStamp + timeStartedPlayback) {
							do {
								...
								userEventLength > 0 &&
								currentTime + 50 >
									self.userEventLog[0].timeStamp + timeStartedPlayback
							);
						}
				...
				if (recordInProgress == true) {
					...
					if (event.type === 'scroll') {
						...
						for (var prop in event) {
							...
							if (
								['number', 'string', 'boolean'].indexOf(typeof event[prop]) > -1 &&
			...
			return {
				...
				*/
				stop: function () {
					...
					window.saveReplayData(JSON.stringify(playbackScript));
				},
			};
		})(),
	};
